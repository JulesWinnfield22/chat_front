...
function CreateGroup() {
    ...
    if(groupName) {
     ...
     let path
      if(pathname == '/creategroup') {
        ...
        let id = pathname.match(/\/editgroup\/(.+)/)?.[1]
        path = 'edit-group'
        groupInfo = {
          ...groupInfo,
          id
      ...
      socket.emit(path, groupInfo, (err, response) => {
        if(err) {
          Object
            .keys(err)
            .forEach(error => {
  ...
  useEffect(() => {
    if(pathname.startsWith('/editgroup')) {
      let id = pathname.match(/\/editgroup\/(.+)/)?.[1]

      let group = groupMessages.find(el => el.id == id)

      let restriction = group.group.restriction
      let type = restriction ? group.group.restriction.type : ''

      if(restriction) {
        setGroupName(group.group.name)
        setGroupDescription(group.group.description || '')
        setGroupNickname(group.group.membersNickname || '')
        setGroupType(group.group.type)
        setRestriction(true)
  ...
  return (
    <>
      <form ref={form} className='w-screen h-screen sm:h-auto sm:w-[22rem] sm:rounded-md m-auto max-h-screen relative overflow-y-auto bg-white flex flex-col'>
        <h2 className='p-2 z-20 sticky top-0 font-medium border-b text-sky-400 bg-gray-800 border-gray-800/40'>{pathname == '/creategroup' ? 'Create Group' : 'Edit Group'}</h2>
        <div className='flex flex-col p-2 gap-1'>
          ...
          <div className='w-full text-sm border rounded-md overflow-hidden'>
            <input value={groupName} onChange={setValue(setGroupName)} id='groupName' className='flex-1 p-2 w-full' placeholder='a verey awesome group' name='groupName' type='text' />
          </div>
          {
            groupNameErr ?
        ...
        <div className='flex flex-col p-2 gap-1'>
          ...
          <div className='w-full text-sm border rounded-md overflow-hidden'>
            <textarea value={groupDescription} onChange={setValue(setGroupDescription)} rows='5' id='groupDescription' className='flex-1 p-2 w-full resize-none' placeholder='the only ones' name='groupDescription' />
          </div>
          {
            groupDescriptionErr ?
        ...
        <div className='flex flex-col p-2 gap-1'>
          ...
          <div className='w-full text-sm border rounded-md overflow-hidden'>
            <input value={groupNickname} onChange={setValue(setGroupNickname)}  id='groupNickname' className='flex-1 p-2 w-full resize-none' placeholder='great ppl' name='groupNickname' />
          </div>
          {
            groupNicknameErr ?
        ...
        <div className='p-2 gap-1 border-t flex w-full'>
          ...
          <Ripple color='#fff8' onClick={create} className='px-4 overflow-hidden border-none py-1 border bg-gray-800 text-sky-400 rounded-md'>
            {pathname == '/creategroup' ? 'create' : 'edit'}
          </Ripple>
        </div>
      </form>
    </>
